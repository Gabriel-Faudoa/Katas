...
describe('tagged template strings, are an advanced form of template strings', function() {
  ...
  //tagFunc was not defined, need to call tagFunction
    });
  ...
  //strings can be in an array
        });
        it('expressions are NOT passed to it', function() {
          var tagged = tagFunction`one${23}two`; 
          assert.deepEqual(tagged, ['one', 'two']);
  //ignores expressions through it and adds a break when it encounters one
        });
      });
  ...
  //needed to call the virst_value parameter not firstValue
        });
  ...
  //... makes it so you can call all of the values out of an array
        });
      });     
    });
